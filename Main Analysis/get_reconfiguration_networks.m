...
%%%         reconfiguration; script contains filtering of connections and 
%%%         computes reconfiguration between connections that remain after 
%%%         the filtering step
%%% Author: Jonas Thiele
%%% Date:   24.06.2021
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% load assignments of nodes to networks and p threshold according to number
% of Yeo networks
if nNetworks == 7
    load Schaf200Yeo7
    node_assignment = Schaf200Yeo7;
    p_thresh = p_thresh_yeo7;
elseif nNetworks == 17
    load Schaf200Yeo17
    node_assignment = Schaf200Yeo17;
    p_thresh = p_thresh_yeo17;
else
...
dist_cosine_join = zeros(size(mask_networks,2), size(combis_states,1),length(subjects));
dist_corr_join = zeros(size(mask_networks,2), size(combis_states,1),length(subjects));
dist_L1_bin_join = zeros(size(mask_networks,2), size(combis_states,1),length(subjects));
...
for k=1:nSamples
    ...
    for c = 1:nStates
        ...
        index_p = find(abs(p_fc) < p_thresh);
    ...
    for c = 1:length(combis_states)
        ...
        corr_mask_scan1_pos = get_correlation_mask(FC_filt, g_score_filt, confounds_filt, p_thresh, state1, 1);
        ...
        corr_mask_scan1_neg = get_correlation_mask(FC_filt, g_score_filt, confounds_filt, p_thresh, state1, -1);
        mask_scan1_neg = corr_mask_scan1_neg.*mask_neg_fil;
    
        corr_mask_scan2_pos = get_correlation_mask(FC_filt, g_score_filt, confounds_filt, p_thresh, state2, 1);
        mask_scan2_pos = corr_mask_scan2_pos.*mask_pos_fil;
    
        corr_mask_scan2_neg = get_correlation_mask(FC_filt, g_score_filt, confounds_filt, p_thresh, state2, -1);
    ...
    for n = 1:size(mask_networks,2) % loop over network combinations
        for c = 1:length(combis_states) % loop over state combinations
